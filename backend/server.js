...
function toNormalizedTask(raw) {
  ...
  return { id: raw.id, title: raw.title, date: dateStr, startTime: d.toISOString(), endTime: endDate.toISOString(), durationMinutes: dur, status: raw.status ?? (raw.completed ? "completed" : "pending"), category: raw.category ?? "general", gatheringId: raw.gatheringId ?? null, isAutoGenerated: Boolean(raw.isAutoGenerated), assignedBy: raw.assigned_by ?? null, room: raw.room ?? null };
}
...
fastify.post("/api/tasks", { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  const assigned_by = actor.actor_name || null;
  const room = typeof body.room === "string" ? body.room.trim() : null;
  const task = { id: nextId(), title, date: dateStr, startTime, endTime, durationMinutes, status: "pending", category, gatheringId, isAutoGenerated, assigned_by, room };
  taskStore.push(task);
...
fastify.patch("/api/tasks/:id", { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  if (typeof body.room === "string") task.room = body.room.trim();
...
// ---- Meal image search ----
// Curated image URLs for menu dishes (reliable, no API needed)
const DISH_IMAGES = {
...
fastify.get("/api/meals/image", { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  if (mealImageCache.has(norm)) {
    return reply.send({ ok: true, url: mealImageCache.get(norm) });
  }

  // Check curated map — try exact match, then partial match
  const curated = DISH_IMAGES[norm] || Object.entries(DISH_IMAGES).find(([k]) => norm.includes(k) || k.includes(norm))?.[1];
  if (curated) {
    mealImageCache.set(norm, curated);
    return reply.send({ ok: true, url: curated });
  }

  const unsplashKey = process.env.UNSPLASH_ACCESS_KEY;

  // Try Unsplash API if key exists
  if (unsplashKey) {
    try {
      const controller = new AbortController();
      ...
      if (url) { mealImageCache.set(dish.toLowerCase(), url); return reply.send({ ok: true, url }); }
    } catch { /* fall through */ }
  }

  // Fallback: loremflickr (free, no key, follows redirect to actual image)
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), 6000);
    const keywords = encodeURIComponent(dish.replace(/[()]/g, "").split(" ").slice(0, 2).join(",") + ",food");
    const res = await fetch(
      `https://loremflickr.com/600/400/${keywords}`,
    ...
    if (location) {
      ...
      mealImageCache.set(dish.toLowerCase(), fullUrl);
      return reply.send({ ok: true, url: fullUrl });
    }
  } catch { /* fall through */ }
